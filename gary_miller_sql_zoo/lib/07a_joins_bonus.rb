...
def top_track_counts
  ...
  execute(<<-SQL)
    SELECT
      albums.title, COUNT(tracks.*) Total
    FROM
      albums
    JOIN tracks ON albums.asin = tracks.album
    GROUP BY albums.title
    ORDER BY Total DESC, albums.title DESC
    LIMIT 10
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, (SUM(counts.price) / SUM(counts.count))
    FROM
      styles
    JOIN (SELECT albums.*, COUNT(tracks.*) AS count
      ...
      GROUP BY albums.asin) AS counts 
        ON styles.album = counts.asin
    GROUP BY styles.style
    ORDER BY
      (SUM(counts.price) / SUM(counts.count)) DESC,
      styles.style ASC
    LIMIT 5
  SQL
end
