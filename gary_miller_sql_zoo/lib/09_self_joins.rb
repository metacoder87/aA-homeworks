...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      b_stops.name, b_routes.company, b_routes.num
    FROM
      routes AS a_routes
    JOIN
      routes AS b_routes ON a_routes.num = b_routes.num
        AND a_routes.company = b_routes.company
    JOIN
      stops AS b_stops ON b_routes.stop_id = b_stops.id
    JOIN
      stops AS a_stops ON a_stops.id = a_routes.stop_id
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT DISTINCT
      start.num, start.company, transfer.name,
